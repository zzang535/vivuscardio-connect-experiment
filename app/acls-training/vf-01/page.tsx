...
function Debrief({ timeline, seconds, onRestart }: { timeline: TimelineItem[]; seconds: number; onRestart: () => void }) {
  ...
  return <main className="min-h-screen bg-[#101820] px-6 py-12 text-slate-100"><div className="mx-auto max-w-4xl"><p className="text-sm font-semibold text-[#8dc4d3]">훈련 완료</p><div className="mt-2 flex items-end justify-between border-b border-[#34414c] pb-6"><div><h1 className="text-3xl font-bold">수행 결과</h1><p className="mt-2 text-sm text-slate-400">각 행동이 언제 수행되었는지 확인하고 대응 흐름을 복습하세요.</p></div><div className="text-right"><p className="text-xs text-slate-500">총 소요 시간</p><p className="mt-1 font-mono text-3xl">{mmss(seconds)}</p></div></div><div className="mt-8 grid gap-6 md:grid-cols-[1fr_240px]"><div className="border border-[#34414c] bg-[#151f28] p-5"><h2 className="font-bold">수행 타임라인</h2><div className="mt-4 divide-y divide-[#2b3741]">{timeline.map((item, index) => <div key={`${item.at}-${index}`} className="grid grid-cols-[56px_12px_1fr] gap-3 py-3"><span className="font-mono text-xs text-slate-500">{mmss(item.at)}</span><span className={`mt-1 h-2 w-2 rounded-full ${item.tone === "good" ? "bg-emerald-500" : item.tone === "warn" ? "bg-amber-500" : "bg-[#6eaabd]"}`} /><span className="text-sm text-slate-300">{item.text}</span></div>)}</div></div><div><dl className="divide-y divide-[#34414c] border-y border-[#34414c]"><div className="flex items-center justify-between py-4"><dt className="text-sm text-slate-400">완료 행동</dt><dd className="text-2xl font-bold text-emerald-400">{timeline.filter((item) => item.tone === "good").length}</dd></div><div className="flex items-center justify-between py-4"><dt className="text-sm text-slate-400">지연·재확인</dt><dd className="text-2xl font-bold text-amber-400">{warnings}</dd></div></dl><button onClick={onRestart} className="mt-6 w-full rounded bg-[#176b87] px-5 py-3.5 font-bold text-white hover:bg-[#145c74]">같은 시나리오 다시 훈련</button><Link href="/acls-training" className="mt-4 block text-center text-sm text-slate-400 hover:text-white">시나리오 목록으로</Link></div></div></div></main>;
}
